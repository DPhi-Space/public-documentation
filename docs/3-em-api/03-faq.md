...
## Docker Build
### I am trying to build a Docker image onboard but the system cannot find the base image
The images onboard are stored by the Docker registry. It has the `fpga.dphi.space:5000` hostname and port, and any Dockerfile building from a base image onboard must have this registry prefixed to the base image it is building from, e.g.:

```bash
FROM fpga.dphi.space:5000/[BASE_IMAGE]
```

All of the public images listed in the [Docker Images onboard](./docs/)
